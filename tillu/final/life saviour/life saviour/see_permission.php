...
$EMAIL=$_GET['email'];
 $conn= mysqli_connect("localhost", "root", "", "LS");
   $q= "select * from CAMP where ID='$ID'";
   $res= mysqli_query($conn, $q);
   $row= mysqli_fetch_assoc($res);
   ?>
   <IMG SRC="<?php echo $row['PERMISSION']?>" style="height: 800px; width: 70%;margin-left: 15%;"><br><br>
 ...
 <script>
   function backfun()
   {
    window.location.href="camp_request_admin1.php";
   }
   function app()
   {
    window.location.href="approve_camp_request.php?id=<?php echo $ID ?>&email=<?php echo $EMAIL ?>";
   }
   function rej()
   {
    window.location.href="reject_camp_request.php?id=<?php echo $ID ?>&email=<?php echo $EMAIL ?>";
   }
 </script>
</body>
</html>
